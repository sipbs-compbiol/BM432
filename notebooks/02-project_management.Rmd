...
- Clean your data in an automated, reproducible way^[We will work with [`OpenRefine`](https://openrefine.org/) as a tool for data cleaning, in the workshop.].
...
```{r finaldoc, fig.cap="How document namestend to grow... [PhD Comics](http://phdcomics.com/comics/archive.php?comicid=1531)"}
...
```{r working-directory, fig.cap="An example working directory structure. For your own work, a different structure may be appropriated"}
...
- `data/` is a subdirectory for stroing data.
  - Should you stare only raw data, or raw and processed data? **That is your decision.**
...
Keeping your raw data in its original state **establishes provenance** and **enables reanalysis**.
...
Keep raw data in a separate, clearly-labelled, subfolder of the project working directory. 
...
<div ide="note">
We will cover one of the tools available for automated and reproducible data-cleaning ([`OpenRefine`](https://openrefine.org/)), in the workshop.
</div>
...
As with data cleaning, manual analysis can sometimes be^[Some would say *frequently is*] not entirely reproducible. Especially when dealing with large datasets, it can be very difficult to be consistent with manual analysis. For very large datasets, manual analysis may not be possible.
...
<div id="note">
Version control is outwith the scope of these workshops, but there are many very good online resources that can help you get started.
</div>
