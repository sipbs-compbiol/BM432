...
In the second notebook ("What Does Statistics Do, Anyway?"), Figure 1.2 showed a plot of a *response variable* against an *explanatory variable*, and a linear regression on that data. This is reproduced below as Figure \@ref(fig:model-relationship).
...
<div id="warning">
We need to always be clear about whether we are considering alternative parameters for the *same model*, or deciding whether alternative models. The two situations are very different.
...
For good science we require that our models are *plausible* and useful: they should represent the mechanism well, but they do not have to fit the data exactly. Models should represent and maybe help explain important features of reality; but we don't expect them to *perfectly* explain data.

We should always try to fit *plausible* models; we should **not** always try to find a model that fits perfectly, and then attempt to bend our understanding of reality to explain the choice of model.
...
<div align="center">
![Spurious correlation between divorce rate in Maine and per capita margarine consumption. $r=0.992$](images/spurious_correlation.png)
</div>
</div>
...
In general, you should always be on guard for spurious correlations - they show up frequently. The main implication is that it is possible to propose a mechanism relating your explanatory and response variables, and "accidentally" find evidence that could support it. This is an example of something called [*confirmation bias*](https://www.bbc.co.uk/bitesize/articles/znbytrd), a logical fallacy that affects us all. Confirmation bias can be quite subtle, and a good way to guard against it - in this context - is to always be asking: "is there another explanation or mechanism that could produce similar results?" This is a kind of *falsification*, a concept we'll return to, later.
...
It is unusual in the natural world, and in experiments, to find *perfect* linear correlation between any two variables, even when a linear relationship describes the relationship well. This is in part because because measurements always involve *uncertainty*, and in part because natural systems tend not to be homogeneous - they are often collections of slightly different individuals; or a single individual may vary slightly over time. If there was truly a linear relationship between two variables, in a real system we wouldn't see a straight line exactly - we would see datapoints scattered about a straight line. The datapoints would not be exactly correlated and, as a result, we find the observed $r^2$value for two variables is usually between 0 and 1, but not exactly 0 or 1.
...
- If the linear model fits perfectly, then the *model variability* will be the same as the *total variability*, our *goodness of fit* equals 1, and we say that the model "explains all the variability in the data."
- If the linear model doesn't describe the data exactly, then *model variability* is less than the *total variability*, our *goodness of fit* is less than 1, and we say that the model "explains some of the variability in the data."

<div id="note">
The *goodness of fit* is often given as a percentage. For instance, if our goodness of fit is 0.73, we might say "the model explains 73% of the variance in the dataset." (Some would say "explains 73% of the data", but this statement is less accurate.)
</div>

This value, *goodness of fit* or *explained variance*, represents uncertainty in the model fit. It recognises that although this is the "optimal" fit for a model with this structure, there is still variability in the data that goes unexplained. Without doing more work, we can't know whether that variability could be captured by improving the model (for example, to represent the mechanism of the system being studied better) or if this is truly *irreducible variability* caused by inexact measurement, or stochastic variation in the system. 
...
This is a different kind of uncertainty than the uncertainty represented by *goodness of fit* above. It does not represent our uncertainty that the "optimal" model describes the data. Instead it describes the uncertainty we have in the *optimal* fitted model parameters, and so **it represents the *uncertainty we have that we found the optimal model, at all***.
